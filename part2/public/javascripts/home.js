    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            for (var i = 0; i < this.responseText.length; i++) {
                document.getElementById('table-body').innerHTML += `<tr> <td>${data[i].dog_name}</td><td>${data[i].owner_username}</td><td>${data[i].size}</td><td><img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTU3HFVnkYFJ_OIogo__Qv58bmhwRqZJcQhOA&s" alt="${data[i].dog_name}">></td>`;
