...
router.get('/api/walkers/summary', function(req, res) {
  try {
    req.pool.getConnection(function(err,connection) {
      ...
      connection.execute(`SELECT User.username AS walker_username, AVG(rating) AS average_rating, COUNT(rating_id) AS total_ratings FROM WalkRatings INNER JOIN Users ON WalkRatings.walker_ID = Users.user_id GROUP BY walker_id`, function (error, results, fields) {
      connection.release();
        if (error) throw error;
          res.send(results);
      });
